...
export default function LogosPage() {
    return (
        <div className="min-h-screen bg-background text-foreground p-5 sm:p-8 lg:p-10 space-y-7 sm:space-y-10 overflow-x-hidden">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Galería de Candidatos de Logo</h1>
            <p className="text-muted-foreground leading-relaxed">Verifica cuál es el correcto para usar en el sitio (Blanco/Transparente).</p>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8">
                {LOGOS.map((logo) => (
                    <div key={logo} className="border border-border rounded-xl p-3 sm:p-4 bg-card/50">
                        <div className="relative aspect-square w-full mb-3 sm:mb-4 bg-transparent border border-border rounded-lg flex items-center justify-center">
                            <Image
                        ...
                        <p className="text-center text-xs font-mono text-muted-foreground break-words">{logo}</p>
                    </div>
                ))}
            </div>
